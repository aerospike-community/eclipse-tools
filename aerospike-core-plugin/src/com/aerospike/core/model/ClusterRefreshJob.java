...
public class ClusterRefreshJob extends Job{
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		try {
			...
			for (AsNode node : this.cluster.nodes.values()){
				...
				if (!namespacesString.isEmpty()){
					...
					for (String nameSpace : nameSpaces){
						AsNameSpace ns = new AsNameSpace(node, nameSpace);
						node.addNameSpaces(ns);
			...
			if (!packagesString.isEmpty()){
				...
				for (String pkgString : packagesList){
					Package pkg = new Package(this.cluster.packages, pkgString);
